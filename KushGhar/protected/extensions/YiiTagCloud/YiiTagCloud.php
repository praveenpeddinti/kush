...
//            $url = isset($conf['url']) ? $conf['url'] : 'javascript:return false';
            ...
            if (!isset($htmlOptions['style']) || empty($htmlOptions['style'])) {
                $htmlOptions['style'] = 'font-size: ' .$conf['font-size'] . 'pt;text-decoration:none;margin-right:15px;';
                if($_SESSION['fromAdminLiteracy'] == "false"){
                    if($conf['eKId'] == $conf['kId']){
                        $htmlOptions['style'] .= "color:green;cursor:pointer;";
                        $htmlOptions['onclick'] = 'deSelectKeyword(\''.$conf['kId'].'\',\''.$conf['font-size'].'\');';
                        $htmlOptions['title'] = "Demote";
                    }else{
                        $htmlOptions['style'] .= "color:".$this->arrFontColor[$conf['font-size']].";cursor:pointer;";
                        $htmlOptions['onclick'] = 'selectKeyword(\''.$conf['kId'].'\',\''.$conf['font-size'].'\');';
                        $htmlOptions['title'] = "Promote";
                    }
                    @$htmlOptions['id'] = 'key_'.$conf['kId'];
                    $tag = $tag.'<sup>'.$conf['weight'].'</sup>';
                }else{
                    if($conf['eKId'] == $conf['kId']){
                        $htmlOptions['style'] .= "color:green;cursor:pointer;";
                        $htmlOptions['onclick'] = 'deSelectAdminKeyword(\''.$conf['kId'].'\',\''.$conf['font-size'].'\');';
                        $htmlOptions['title'] = "Demote";
                    }else{
                        $htmlOptions['style'] .= "color:".$this->arrFontColor[$conf['font-size']].";cursor:pointer;";
                        $htmlOptions['onclick'] = 'selectAdminKeyword(\''.$conf['kId'].'\',\''.$conf['font-size'].'\',\''.$this->arrFontColor[$conf['font-size']].'\');';
                        $htmlOptions['title'] = "Promote";
                    }
                    @$htmlOptions['id'] = 'adminkey_'.$conf['kId'];
                }
...
//                $htmlOptions['target'] = '_blank';
    ...
    public function getMinAndMaxWeight() {
        ...
        foreach ($this->arrTags as $conf) {            
            ...
            error_log("==getMinAndMaxWeight===weight====".$conf['weight']."==max==".$this->maxWeight."=======min====".$this->minWeight);
        }
    }
    ...
    public function calcFontSize($weight) {
		...
		return round(((($weight - $this->minWeight) * ($this->maxFontSize - $this->minFontSize)) / ($this->maxWeight - $this->minWeight)) + $this->minFontSize);
            ...
            error_log("@@@@@@@@@=====Error occurred==YII Tagcloud==========@@@@@@@@@@@@@@@".$cloudex->getMessage());
        }
    }
