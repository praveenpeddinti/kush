...
class coActiveMailServer {
   ...
   function __construct($obj) {
        ...
        }else if ($obj['mailType'] == "CustomerInvitationMailToKGTeam") {
            //$this->getInvitationMail($obj, $host, $port, $authMail, $authPassword, $coActiveLogo,$date);
            $this->getCustomerInvitationMailToKGTeam($obj, $host, $port, $authMail, $authPassword, $coActiveLogo,$date);
        }
...
//        }
    ...
    function getCustomerInvitationMailToKGTeam($obj, $host, $port, $authMail, $authPassword,$coActiveLogo,$date) {
        error_log($coActiveLogo."==&*****************&&&&&&&&&&&&&&&&********************in dependentMessage function=========");
        
        $template2 = '';

        $temp2 = "/opt/lampp/htdocs/KushGhar/protected/extensions/mail/CustomerInvitationMailToKGTeam.html";
        $handle2 = fopen($temp2, "r");
        $data2 = file_get_contents($temp2);
        $addAddress = $obj['toAddress'];
        $cust_email = $obj['subject'];
        $userName = $obj['userName'];
        $message = $obj['message'];
        $Kushmessage2 = "KushGhar - Making people's lives better, one home at a time.";
        $employerName = $obj['employerName'];
...
//        for ($i = 0; $i < sizeof($addAddress); $i++) {
            if (trim($addAddress) != "") {
                try {
                    ...
                    $template2 = str_replace('{--USERNAME--}',stripslashes(ucwords("$userName")),$data2);                    
                    $template2 = str_replace('{--DATE--}',stripslashes($date),$template2);
                    $template2 = str_replace('{--EMAIL--}',stripslashes($cust_email),$template2);
                    ...
                    //$mail->AddCC('rtummala1@yahoo.com');
                    //$mail->AddCC('swamy.deva1@gmail.com');
                    //$mail->AddCC('satyalika@gmail.com');
                    $mail->AddCC('jamma.suresh@gmail.com');
//                    $mail->AddBCC('admin@kushghar.com');
//                    $mail->AddBCC('helpme@kushghar.com');
                    ...
                    $mail->Subject = 'New KushGhar Invitation Received';
