 ...
 * 
 */
class YiiTagCloud_new extends CWidget
{
    /**
...
//            $url = isset($conf['url']) ? $conf['url'] : 'javascript:return false';
            ...
            if (!isset($htmlOptions['style']) || empty($htmlOptions['style'])) {
                $htmlOptions['style'] = 'font-size: ' .$conf['font-size'] . 'pt;cursor:pointer;' . 'color: ' . $this->arrFontColor[$conf['font-size']];
            }

            if (!isset($htmlOptions['target']) || empty($htmlOptions['target'])) {
//                $htmlOptions['target'] = '_blank';
            }
            $htmlOptions['id'] = "promotedTags_".$i++;
            error_log("=======keyword from cloud====".$conf['keyword']);
            $htmlOptions['onclick'] = "searchRelatedTags('".$conf['keyword']."')";
